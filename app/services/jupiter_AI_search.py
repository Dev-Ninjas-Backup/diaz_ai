...
class SQLiteQueryAgent:
    ...
    def __init__(self, db_path: str, table_name: str = "boats"):
    ...
    def execute_query(self, user_query: str, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            sql_query = self._generate_sql(user_query, limit)
    ...
    def _generate_sql(self, user_query: str, limit: int) -> str:
        """Generate SQL query using OpenAI (safer than code execution)."""
        
        system_prompt = """You are a SQL expert. Generate SQLite queries for a boat database.
...
Example queries:
- "boats under 500000" → SELECT * FROM boats WHERE price < 500000
- "boats in Miami" → SELECT * FROM boats WHERE LOWER(city) LIKE '%miami%' OR LOWER(location) LIKE '%miami%'
- "2024 Freeman boats" → SELECT * FROM boats WHERE model_year = 2024 AND LOWER(make) LIKE '%freeman%'
- "boats longer than 40 feet" → SELECT * FROM boats WHERE length_overall > 40
- "most expensive boats" → SELECT * FROM boats ORDER BY price DESC
- "boats with 2 engines" → SELECT * FROM boats WHERE number_of_engines = 2
"""
